...
public class MysqlItemReader {
    ...
    public MultiResourceItemReader<StoreRequestDto> multiResourceItemReader() {
        ...
        Resource[] resources = Arrays.stream(Objects.requireNonNull(folder.listFiles((dir, name) -> name.endsWith(".csv"))))
                .map(FileSystemResource::new)
                .toArray(Resource[]::new);

        return new MultiResourceItemReaderBuilder<StoreRequestDto>()
                .name("multiCsvReader")
                .resources(resources)
