...
public class BrandNameController {
    ...
    @PostMapping("/brand")
    public ResponseEntity<BaseResponse<List<String>>> generateBrandName(@RequestBody BrandNameRequestDto brandNameRequestDto) {
        try {
            List<String> response = openAiService.getBrandName(brandNameRequestDto).get(); // get()을 호출해 응답을 동기적으로 기다립니다.
